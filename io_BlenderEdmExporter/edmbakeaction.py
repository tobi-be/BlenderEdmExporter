...
def edmbake(con):
    ...
    for i in range(action.EDMBakeStartFrame,action.EDMBakeEndFrame+1):
        ...
        for name in rotationkeyframedBones:
            armature.keyframe_insert('pose.bones["'+name+'"].rotation_quaternion',index=-1, frame=i,group=name,options={'INSERTKEY_VISUAL'})
        for name in scalekeyframedBones:
            armature.keyframe_insert('pose.bones["'+name+'"].scale',index=-1, frame=i,group=name,options={'INSERTKEY_VISUAL'})
        for name in locationkeyframedBones:
            armature.keyframe_insert('pose.bones["'+name+'"].location',index=-1, frame=i,group=name,options={'INSERTKEY_VISUAL'})
